...
// container, using the signed sandbox entitlement on macOS.
//
...
// container, using the signed sandbox entitlement on macOS. Its presence is the signal that access has to be negotiated.
...
type Granter interface {
	...
	// Restore uses only a saved grant and never displays a picker.
	Restore(path string) (Release, error)
...
func (passthrough) Restore(string) (Release, error) { return noRelease, nil }
