...
//
// Compiled and exercised on macOS in standalone and actual Godot sandbox probes.
// Distribution signing and other credential workflows still require separate validation.
...
func (g *bookmarkGranter) prompt(ctx context.Context, path string) (Release, error) {
	...
	if resolved, e := filepath.EvalSymlinks(requested); e == nil {
		requested = resolved
	}
	relative, err := filepath.Rel(granted, requested)
	if err != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
		return noRelease, fmt.Errorf("sandbox: select the folder containing the requested file")
	}
...
func runOpenPanel(ctx context.Context, dir string) (foundation.URL, bool, error) {
	...
	show := func() {
		...
		panel.SetMessage(fmt.Sprintf(
			"Bufflehead needs access to %s to read your files.", dir))
...
// Restore must never prompt: callers include the external control API.
func (g *bookmarkGranter) Restore(path string) (Release, error) {
	_, bookmark, ok := g.store.Lookup(path)
	if !ok {
		return noRelease, fmt.Errorf("sandbox: open this folder in Bufflehead first to grant access")
	}
	release, _, err := startAccess(bookmark)
	return release, err
}

func (g *bookmarkGranter) scopeKey(path string) string {
	if dir, _, ok := g.store.Lookup(path); ok {
		return dir
	}
	return path
}
