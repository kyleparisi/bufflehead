...
func RunOpenDB(dbPath string, tabID, generation uint64, cmd *control.Command, results chan DBResult) {
	runOpenDBWith(func(p string) (db.Querier, error) { return db.OpenDB(p) },
		dbPath, tabID, generation, cmd, results)
}

// RunOpenSQLite is the SQLite counterpart to RunOpenDB: it opens the file
// natively via the pure-Go modernc.org/sqlite driver (no DuckDB extension). The
// result flows through the same ReqOpenDB path, so the connection is created
// identically.
func RunOpenSQLite(dbPath string, tabID, generation uint64, cmd *control.Command, results chan DBResult) {
	runOpenDBWith(func(p string) (db.Querier, error) { return db.OpenSQLiteNative(p) },
		dbPath, tabID, generation, cmd, results)
}
...
func runOpenDBWith(open func(string) (db.Querier, error), dbPath string, tabID, generation uint64, cmd *control.Command, results chan DBResult) {
	go func() {
